...
Most Proxmox MCPs make you pick: a read-only toy, or full keys and pray. I wasn't going to pick. Proximo plans every dangerous move first, snapshots before it acts where the platform allows, and writes every action to a tamper-evident ledger you hold. 908 tools across all four Proxmox products: Virtual Environment, Backup Server, Mail Gateway, Datacenter Manager. One trust core over MCP, A2A, and HTTP. New in 0.40.0: the doctor says where the shell lands. ct_exec and the node shell ride ssh or run on the box Proximo runs on, never the API, so proximo doctor now names the machine they land on and flags a split target when that is not the API host, naming both machines and every feature that rides the target. Names for one machine count as one, resolved through ssh's own config and by address, never DNS. v0.40.0 on PyPI (`uvx proximo-proxmox`), GitHub, and GHCR (signed image). Apache 2.0.
