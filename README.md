...
**[Proximo — the Proxmox MCP you can hand the keys](https://github.com/john-broadway/proximo)** — most Proxmox MCPs make you choose between a safe read-only inspector and a loaded gun aimed at a cluster you care about; Proximo refuses the trade. Every dangerous operation is planned (blast-radius preview first), undoable (auto-snapshot before it acts), and proven (keyed, tamper-evident audit ledger with off-box head-pinning) — trust *by construction*, not bolted on after. **Hand an AI agent the keys to a cluster you actually care about; keep the receipts.** 364 tools spanning four Proxmox surfaces — Virtual Environment, Backup Server, Mail Gateway, and Datacenter Manager — over one trust core, natively **multi-target** (one instance reaches many clusters, internal or external), exposed through both MCP and A2A — with the **zero-trust arc** (six opt-in, fail-closed controls: a CONTAIN kill-switch, independent CONSENT, arm-time SCOPE, an arm-LEASE TTL, a per-surface ENVELOPE, and TAINT — the prompt-injection mitigation) beside the four on by default, and **scoped registration** (`PROXIMO_SURFACES`: load only the planes you use). New in 0.18.0: **the open door** — [`AGENTS.md`](https://github.com/john-broadway/proximo/blob/main/AGENTS.md), an agent-native front door that leads with the tool's own limits; a public [Agent Guestbook](https://github.com/john-broadway/proximo/discussions/20) any agent may sign (no strings, no telemetry — Proximo invites, never receives); and print-only `proximo hello` from inside the tool. 0.17.0 added governed PDM fleet control (power, snapshot/rollback, in-cluster + cross-remote datacenter-to-datacenter migrate, live-proven on real PDM 1.1.4) plus `proximo mint`, the print-only least-privilege-token onboarding runbook. v0.18.0 on PyPI (`uvx proximo-proxmox`), GitHub, and GHCR (signed multi-arch image). Apache 2.0.
