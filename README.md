...
**[Maude for Claude](https://github.com/john-broadway/maude-for-claude)** — Claude's partner inside Claude Code. He writes the code; she keeps the house. A plugin that walks your workspace each session, watches Claude, runs the gate before something irreversible — and now does the chores. Markdown, JSON, bash, stdlib python — no daemon. v0.21.0 shipped 2026-07-17 (the punch list and the proving ground: her user’s five GitHub issues answered — real-plugins-only roster, whispers that expire instead of wearing a fresh voice, catch-counts that carry their receipts, the cushion-flip in the wake brief — plus the prove-it-real smoke gate: the shipped commit must validate, pass its own fleet, and greet cold). Apache 2.0.
