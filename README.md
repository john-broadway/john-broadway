...
Most Proxmox MCPs make you choose between a safe read-only inspector and a loaded gun aimed at a cluster you care about; Proximo refuses the trade. Every dangerous operation is planned (blast-radius preview first), undoable (snapshots before it acts, where the platform can), and proven (keyed, tamper-evident audit ledger). 900 tools across the four Proxmox surfaces — Virtual Environment, Backup Server, Mail Gateway, Datacenter Manager — one trust core, MCP (stdio + streamable HTTP) + A2A + HTTP/OpenAPI, natively multi-target. New in 0.25.0: the Mail Gateway plane closes — the full PVE + PBS + PMG + PDM surface on one trust core — and a fourth transport arrives, native MCP over Streamable HTTP (a first community contribution). v0.25.0 on PyPI (`uvx proximo-proxmox`), GitHub, and GHCR (signed multi-arch image). Apache 2.0.
