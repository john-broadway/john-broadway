...
Most Proxmox MCPs make you pick: a read-only toy, or full keys and pray. I wasn't going to pick. Proximo plans every dangerous move first, snapshots before it acts where the platform allows, and writes every action to a tamper-evident ledger you hold. 908 tools across all four Proxmox products: Virtual Environment, Backup Server, Mail Gateway, Datacenter Manager. One trust core over MCP, A2A, and HTTP. New in 0.39.0: the junction gets its law. The guest shell lane answered to no PVE privilege; now the served token's own permission map governs it, asked of PVE per guest path and fail-closed, and every change to reach on either side lands in the ledger. v0.39.0 on PyPI (`uvx proximo-proxmox`), GitHub, and GHCR (signed image). Apache 2.0.
