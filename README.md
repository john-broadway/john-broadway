...
Most Proxmox MCPs make you pick: a read-only toy, or full keys and pray. I wasn't going to pick. Proximo plans every dangerous move first, snapshots before it acts where the platform allows, and writes every action to a tamper-evident ledger you hold. 908 tools across all four Proxmox products: Virtual Environment, Backup Server, Mail Gateway, Datacenter Manager. One trust core over MCP, A2A, and HTTP. New in 0.41.1: the security patch layer only ran on a digest bump. The image has applied Debian's patches at build time since July, but every build passed a shared layer cache and that step never changed, so BuildKit replayed it: the layer only ever re-ran when the pinned base digest moved, never because a fix shipped. 0.41.0 carried a 2026-09-04 layer byte-identical to 0.40.0's, so both scanned at 3 critical and 9 high base-OS CVEs that Debian 13.7 fixed on 2026-09-12. The layer rebuilds every run now, held by a test at every build site. v0.41.1 on PyPI (`uvx proximo-proxmox`), GitHub, and GHCR (signed image). Apache 2.0.
...
Claude's partner inside Claude Code. He writes the code; she keeps the house. She walks the workspace each session, watches Claude, and runs the gate before something irreversible. Markdown, JSON, bash, stdlib Python. No daemon, no baggage. v0.31.0 shipped 2026-09-13 (the law arrives at the moment the work touches its class: three bodies of design law ship as data, and a rail called from the hooks that already fire says the law once on the first write of a class, lints a schema on the spot for the shapes its own text can break, and asks at the commit if no design named its laws). Apache 2.0.
...
<img width="362" height="380" alt="John Broadway" src="https://github.com/user-attachments/assets/d4c40d75-d6a2-4a21-a101-a88582cfb155" />
