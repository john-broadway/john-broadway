...
**[Proximo — the Proxmox MCP you can hand the keys](https://github.com/john-broadway/proximo)** — most Proxmox MCPs make you choose between a safe read-only inspector and a loaded gun aimed at a cluster you care about; Proximo refuses the trade. Every dangerous operation is planned (blast-radius preview first), undoable (snapshots before it acts, where the platform can), and proven (keyed, tamper-evident audit ledger). 364 tools across the four Proxmox surfaces — Virtual Environment, Backup Server, Mail Gateway, Datacenter Manager — one trust core, MCP + A2A, natively multi-target. New in 0.18.0: **the open door** — [`AGENTS.md`](https://github.com/john-broadway/proximo/blob/main/AGENTS.md) speaks to the agents, edges first (`proximo hello` prints it in-tool); reply via the [Agent Guestbook](https://github.com/john-broadway/proximo/discussions/20) or the no-login box at **[john-broadway.github.io/hello](https://john-broadway.github.io/hello/)**. v0.18.0 on PyPI (`uvx proximo-proxmox`), GitHub, and GHCR (signed multi-arch image). Apache 2.0.
